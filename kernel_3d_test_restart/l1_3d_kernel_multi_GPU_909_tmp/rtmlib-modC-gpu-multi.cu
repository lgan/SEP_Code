...
{	
	...
	for(int i=1; i<n_gpus; i++){
		...
		add_source_kernel<<<dimGrid_source, dimBlock>>>(ny, nz, nx, 25, 100, 100, g_it, g_ex_sigmayy0_now[1], g_ex_sigmaxx0_now[1], g_ex_sigmazz0_now[1]);
		
		err = cudaGetLastError();
		if(cudaSuccess != err){
			fprintf(stderr, "Cuda error source: %s.\n", cudaGetErrorString(err));
			exit(0);
		}
...
#else
	fprintf(stderr,"Single GPU Implementation has been moved out from this project, please refer to previous backup\n");
#endif
}
