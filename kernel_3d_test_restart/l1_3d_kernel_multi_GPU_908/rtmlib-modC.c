...
//check an array and print the index and value of the non-zero point
void func_print_nz(int ny, int nx, int nz, float *a){
	int iy, iz, ix;
	int flag = 1;
	for(iy = 0; iy < ny; iy++)
		for(ix = 0; ix < nx; ix++)
			for(iz = 0; iz < nz; iz++){
				if(a[n3d_index_ex(iz,ix, iy)]){
//					fprintf(stderr, "%d,%d,%d, %.14f\n",iy,ix, iz,   0);// a[n3d_index_ex(iz,ix, iy)] );
	...
	if(flag) fprintf(stderr, "Non-zero point not found\n");
	else 	 fprintf(stderr, "Non-zero point found\n");
...
int main(int argc, char *argv[]){
	...
	//fprintf(stderr, "Check before model_op\n");
	
//	func_print_nz(ny, nx, nz, ex_m2);
//	func_print_nz(ny, nx, nz, ex_m3);
//	func_print_nz(ny, nx, nz, ex_m2m3);
//	func_print_nz(ny, nx, nz, ex_m1_x);
//	func_print_nz(ny, nx, nz, ex_m1_y);
//	func_print_nz(ny, nx, nz, ex_m1_z);
...
//	func_print_nz(ny, nx, nz, ex_m1_z);
	...
	//fprintf(stderr, "Model feild Initallized ==========> OK\n");
#ifdef GPU
	fprintf(stderr, "\n\n***********************************************\n");
...
//*************************************
	...
	int err0=srite("data_source",source,sizeof(float)*nt);
	//return NULL;
...
for(it = 0 ; it<nt; it++){
	...
	fprintf(stderr,"Write to File Vx0...\n");
	//FILE *fp=fopen("Vx0","rw+");
	...
	ierr = srite("data_Vz0",ex_Vz0_pre, sizeof(float)*(nx+10)*(ny+10)*(nz+10));
		...
		//Add source and Change pointer	
	if(it < nt-1){

		

		//Add Source
		add_source(source[it], 100, 100, 100, ny, nx, nz, ex_sigmayy0_now);
		add_source(source[it], 100, 100, 100, ny, nx, nz, ex_sigmaxx0_now);
		add_source(source[it], 100, 100, 100, ny, nx, nz, ex_sigmazz0_now);
	...
	func_print_nz(ny, nx, nz, ex_sigmaxx0_pre);
	func_print_nz(ny, nx, nz, ex_sigmayy0_pre);
	func_print_nz(ny, nx, nz, ex_sigmazz0_pre);
	func_print_nz(ny, nx, nz, ex_Vx0_pre);
	func_print_nz(ny, nx, nz, ex_Vy0_pre);
	func_print_nz(ny, nx, nz, ex_Vz0_pre);
	func_print_nz(ny, nx, nz, ex_sigmaxy0_pre);
	func_print_nz(ny, nx, nz, ex_sigmaxz0_pre);
	func_print_nz(ny, nx, nz, ex_sigmayz0_pre);
