...
{	
	...
	int start_back[n_gpus];	//index for copy data to CPU
	...
	for(int i=0; i<n_gpus; i++){
	cudaSetDevice(i);
	dim3 dimGrid(n1/TZ, n2/TX, n3/TY);
	dim3 dimBlock(TZ, TX, TY);

	
		fprintf(stderr, "GPU id %d\n", i);
		rtm_gpu_kernel<<<dimGrid, dimBlock>>>(ny, nz, nx,
			g_ex_Vy0_in, g_ex_Vx0_in[i], g_ex_Vz0_in, g_ex_sigmayy0_in[i], g_ex_sigmaxx0_in[i], g_ex_sigmazz0_in[i], g_ex_sigmaxy0_in[i], g_ex_sigmaxz0_in[i], g_ex_sigmayz0_in[i],
			g_ex_Vy0_in1, g_ex_Vx0_in1[i], g_ex_Vz0_in1, g_ex_sigmayy0_in1, g_ex_sigmaxx0_in1, g_ex_sigmazz0_in1, g_ex_sigmaxy0_in1, g_ex_sigmaxz0_in1, g_ex_sigmayz0_in1,
			g_ex_Vy0_out, g_ex_Vx0_out[i], g_ex_Vz0_out, g_ex_sigmayy0_out, g_ex_sigmaxx0_out, g_ex_sigmazz0_out, g_ex_sigmaxy0_out, g_ex_sigmaxz0_out, g_ex_sigmayz0_out,
			g_ex_m1_y[i], g_ex_m1_x[i], g_ex_m1_z[i], g_ex_m2[i], g_ex_m3[i], g_ex_m2m3[i]);
	}
