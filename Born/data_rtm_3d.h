...
class data_rtm_3d: public wavefield_insert_3d{
 ...
 void get_source_func(hypercube_float *domain, int ishot, float *s_x, float *s_y, float *s_z, int nsinc,  int nts, hypercube_float *time);
 //void get_source_func_encode(hypercube_float *domain, int ishot, bool encode, int *rvec, float *s_z, float *s_x, float *s_y, int nsinc,  int nts, hypercube_float *time);
  void set_source_file(oc_float *pntr){
     myf=pntr;
  }

  int nshotsx(){return get_axis(4).n;}
  int nshotsy(){return get_axis(5).n;}
  int nshots(){return get_axis(4).n*get_axis(5).n;}
